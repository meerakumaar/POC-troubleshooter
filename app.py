...
st.set_page_config(
    page_title="POC Troubleshooter (RAG Prototype)",
    layout="centered"
)
...
# Show what Streamlit actually sees (keep this)
with st.expander("Loaded manuals (debug)"):
    if not manual_texts:
        st.write("No manuals found.")
...
if total_chars < 1500:
    st.warning(
        ...
        "If this persists, Streamlit may still be deploying an old build."
    )

# ============================
# RAG: CHUNKING + EMBEDDINGS
# ============================
def chunk_text(text: str, size: int = 1400, overlap: int = 150):
    chunks = []
    i = 0
    while i < len(text):
...
def build_index(manuals: dict):
    ...
    res = client.models.embed_content(model=EMBED_MODEL, contents=contents)

    vecs = [vec_from_embedding(e) for e in res.embeddings]
    mat = normalize(np.vstack(vecs))
...
def grounded_response(query: str) -> str:
    ...
    if not hits or hits[0][2] < 0.22:
        return (
            "Step 1:\n"
            "Need more info.\n"
            "What exact error message or behavior is shown on the device?"
        )
...
user_input = st.chat_input(
    "Describe the issue (include device + error code if possible)"
)
